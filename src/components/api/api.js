...
export const imageUrl = function (
  dimension = "w370_and_h556_bestv2"
) {
...
export async function getExternalLinks(mediaType, id) {
  const data = await fetch(
    `https://api.themoviedb.org/3/${mediaType}/${id}/external_ids?api_key=${apiKey}`
  ).then((res) => res.json());
  return data;
}
...
export async function getRecommandations(mediaType, id) {
  const data = await fetch(
    `https://api.themoviedb.org/3/${mediaType}/${id}/recommendations?api_key=${apiKey}&language=en-US&page=1`
  ).then((res) => res.json());
  return data;
}
...
export async function getDurationVideo(id) {
  const data = await fetch(
    `https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${id}&key=${youtubeKey}`
  ).then((res) => res.json());
