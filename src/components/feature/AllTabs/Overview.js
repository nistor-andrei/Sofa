...
import { SlideCast } from '../../Slide/SlideCast';
import { Footer } from '../../Footer/Footer';
import { SlideRecommand } from '../../Slide/SlideRecommand';

export function Overview({ media }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    async function getData() {
      const links = await getExternalLinks(media, id);
      const data = await getMovie(media, id);
      setOverview(data);
      setLinks(links);
    }
  ...
  let date;
  if (overview.release_date) {
    date = overview.release_date.split('-').reverse().join('.');
  } else {
    date = overview.first_air_date.split('-').reverse().join('.');
  }
  ...
  return (
    <>
        ...
        <div className={styles.infoRight}>
          ...
          <ul className={styles.tabsContent}>
            ...
            <li>
              ...
              <p>{timeConvert(overview.runtime || overview.episode_run_time)}</p>
            </li>
            {
            ...
            {overview.number_of_seasons && (
              <li>
                <p className={styles.titleDetails}> Seasons</p>
                {overview.number_of_seasons}
              </li>
            )}
            {overview.number_of_episodes && (
              <li>
                <p className={styles.titleDetails}> Episodes</p>
                {overview.number_of_episodes}
              </li>
            )}
            <li>
            ...
            <li>
              <a href={`https://www.imdb.com/title/${overview.imdb_id || links.imdb_id}/`}>
                <svg
                  aria-hidden="true"
                  focusable="false"
                  data-prefix="fab"
      ...
      <section className={styles.sliders}>
        <SlideCast mediaType={media} id={id} title="Cast" />
      </section>
      <section className={styles.sliders + ' space'}>
